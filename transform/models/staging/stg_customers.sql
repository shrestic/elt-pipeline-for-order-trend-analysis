...
customer as (
    select  {{ remove_double_quote('customer_id') }} as customer_id, 
            {{ remove_double_quote('customer_unique_id') }} as customer_unique_id, 
            {{ remove_double_quote('customer_zip_code_prefix') }} :: int  as customer_zip_code_prefix, 
            customer_city, 
            customer_state,
            ROW_NUMBER() OVER(
...
select  customer_id, 
        ...
        customer_state
from customer
where (customer_id,customer_unique_id, rw_num) IN (
